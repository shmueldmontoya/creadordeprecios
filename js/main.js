...
/**
 * Valida si un número es válido (no NaN y mayor que 0)
 * @param {number} valor - Valor a validar
 * @returns {boolean} True si es válido
 */
const esPrecioValido = (valor) => !isNaN(valor) && valor > 0;

/**
 * Formatea el precio con el símbolo de colón
 * @param {number} precio - Precio a formatear
 * @returns {string} Precio formateado
 */
const formatearPrecio = (precio) => {
  if (!window.APP_CONFIG) return `₡${precio}`;
  return window.Utils.formatearPrecio(precio);
};
...
const mostrarError = (mensaje) => {
  // Usar el mismo sistema de notificaciones que los atajos de teclado
...
const limpiarFormulario = () => {
  ...
  mostrarNotificacion('Formulario borrado', 'success');
};
...
const actualizarRotulo = () => {
  ...
  if (actual > 0) {
    const precioFormateado = formatearPrecio(actual);
  ...
  if (anterior > 0 && ahorro > 0) {
    DOM_ELEMENTS.textoAnterior.textContent = `Antes: ${formatearPrecio(anterior)}`;
    DOM_ELEMENTS.textoAhorre.textContent = `Ahorra: ${formatearPrecio(ahorro)}`;
  } else {
...
const descargarImagen = async () => {
  ...
  try {
    ...
    // Descargar imagen
    const nombreArchivo = window.Utils.generarNombreArchivo('rotulo', 'png');
    window.Utils.descargarCanvas(canvas, nombreArchivo);
...
const validarDatosRotulo = (datos) => {
  ...
  if (!esPrecioValido(datos.actual)) {
    mostrarError('Debes ingresar al menos el precio actual.');
    return false;
  }
  
  if (esPrecioValido(datos.anterior) && datos.ahorro <= 0) {
...
const agregarRotuloACola = () => {
  ...
  if (APP_STATE.editandoIndice !== null) {
    ...
    mostrarNotificacion('Rótulo editado correctamente', 'success');
  } else {
    // Agregar a la cola
    APP_STATE.colaRotulos.push(datos);
    mostrarNotificacion('Rótulo agregado a la cola', 'success');
  }
...
const eliminarRotuloDeCola = (indice) => {
  ...
  actualizarInterfazCola();
  mostrarNotificacion('Rótulo eliminado de la cola', 'info');
};
...
const limpiarCola = () => {
  ...
  actualizarInterfazCola();
  mostrarNotificacion('Cola de rótulos limpiada', 'info');
};
...
const crearElementoRotulo = (rotulo, indice) => {
  ...
  if (rotulo.actual) {
    const precioFormateado = formatearPrecio(rotulo.actual);
    if (rotulo.unidad) {
...
const descargarLoteRotulos = async () => {
  ...
  if (colaRotulos.length === 0) {
    mostrarError('No hay rótulos en la cola para descargar.');
    return;
  }

  try {
    mostrarNotificacion('Generando rótulos...', 'info');
    ...
    if (!JSZip) {
      ...
      mostrarNotificacion('Todos los rótulos han sido descargados', 'success');
      return;
    }
    ...
    for (let i = 0; i < colaRotulos.length; i++) {
      ...
      const nombreArchivo = `${rotulo.producto}-${i + 1}.png`;
      zip.file(nombreArchivo, canvas.toDataURL().split(',')[1], {base64: true});
    }
    ...
    mostrarNotificacion('Lote de rótulos descargado exitosamente', 'success');

  } catch (error) {
    console.error('Error al descargar lote:', error);
    mostrarError('Error al generar el lote de rótulos.');
  }
};
...
const generarCanvasRotulo = async (rotulo) => {
  ...
  if (esPrecioValido(rotulo.actual)) {
    const precioFormateado = formatearPrecio(rotulo.actual);
    if (rotulo.unidad) {
  ...
  if (esPrecioValido(rotulo.actual) && esPrecioValido(rotulo.anterior)) {
    textoAnterior.textContent = `Precio regular: ${formatearPrecio(rotulo.anterior)}`;
    textoAhorre.textContent = `Ahorro: ${formatearPrecio(rotulo.ahorro)}`;
...
const descargarRotuloIndividual = async (rotulo, indice) => {
  try {
    const canvas = await generarCanvasRotulo(rotulo);
    const nombreArchivo = `${rotulo.producto}-${indice + 1}.png`;
    window.Utils.descargarCanvas(canvas, nombreArchivo);
  } catch (error) {
...
const cargarBaseDatos = async () => {
  try {
    ...
    mostrarError('Error al cargar la base de datos de productos.');
  }
};
...
function inicializarModalEditarRotulo() {
  ...
  form.onsubmit = function(e) {
    ...
    const datos = {
      ...
      unidad: document.getElementById('editarKg').checked ? 'kg' : (document.getElementById('editarUd').checked ? 'ud' : ''),
    ...
    if (!datos.actual || datos.actual <= 0) {
      mostrarError('El precio actual es obligatorio y debe ser mayor que 0');
      return;
    }
    if (datos.anterior && datos.anterior <= datos.actual) {
      mostrarError('El precio anterior debe ser mayor que el precio actual');
      return;
    }
    if (APP_STATE.editandoIndice !== null) {
      APP_STATE.colaRotulos[APP_STATE.editandoIndice] = datos;
